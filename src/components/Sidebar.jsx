...
function Sidebar() {
    ...
    return (
        <div className="fixed top-0 left-0 max-sm:h-20 min-sm:h-screen max-sm:w-screen z-50 flex" onMouseEnter={() => setOpen(true)} onMouseLeave={() => setOpen(false)}>
            <div className={` w-20 bg-secondary max-sm:w-screen sm:h-screen max-sm:flex max-sm:items-center`}>
...
const SideBarIcon = ({link, icon, title, open }) => (
    <Link to={link}>
        <div className={`max-sm:ml-4 max-sm:mb-5 max-sm:mx-2.5 max-sm:w-16 flex items-center`}>
            <button className="sidebar-icon sm:ml-1.5">
                {icon}
            </button>
            <span className={`max-sm:invisible ml-3 text-base font-medium text-primary group-hover:text-button_small duration-150`}></span>
        </div>
    </Link>
);
